...
func searchHandler(c echo.Context) error {
	...
	documents, err := GetDocuments("../server/data/test_100k.csv", docIds, globalDocIndex)
	if err != nil {
		return echo.NewHTTPError(http.StatusInternalServerError, err.Error())
	}
...
func saveAllObjects() error {
	...
	} else if err := os.WriteFile("../server/data/lexicon.json", lexiconBytes, 0644); err != nil {
	...
	} else if err := os.WriteFile("../server/data/document_index.json", docIndexBytes, 0644); err != nil {
	...
	} else if err := os.WriteFile("../server/data/barrel_metadata.json", barrelMetaBytes, 0644); err != nil {
	...
	} else if err := os.WriteFile("../server/data/metadata.json", metadataBytes, 0644); err != nil {
...
func updateBarrel(termId string, docId string, data struct {
	...
	barrelPath := "../server/data/barrels/barrel_" + strconv.Itoa(barrelId) + ".json"
...
func saveBarrelMetadata() error {
	...
	return os.WriteFile("../server/data/barrel_metadata.json", metadataBytes, 0644)
}
...
func appendDocumentToDataset(doc ClientDocumentType, docId string) error {
	...
	file, err := os.OpenFile("../server/data/test_100k.csv", os.O_APPEND|os.O_WRONLY, 0644)
	if err != nil {
