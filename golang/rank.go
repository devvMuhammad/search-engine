...
func loadMetadata() (*Metadata, error) {
	data, err := os.ReadFile("../server/data/metadata.json")
	if err != nil {
		return nil, err
	}
...
func loadBarrelMetadata() (BarrelMetadata, error) {
	data, err := os.ReadFile("../server/data/barrel_metadata.json")
	if err != nil {
...
func getDocs(barrel_number int, wordID string) ([]Posting, error) {
	data, err := os.ReadFile(fmt.Sprintf("../server/data/barrels/barrel_%d.json", barrel_number))
	if err != nil {
...
func loadLexicon() map[string]Lexicon {
	data, err := os.ReadFile("../server/data/lexicon.json")
	if err != nil {
		return nil
	}
...
func main2() {
	...
	docTime := time.Now()
	documentData, docError := GetDocuments("../server/data/test_100k.csv", docIds[:50], documentIndex)
